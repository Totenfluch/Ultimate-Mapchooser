...
Handle:BuildNominationMenu(client, const String:cat[] = INVALID_GROUP)
{
	...
	//Copy over for template processing
	dispKV = CreateKeyValues("umc_mapcycle");
	KvCopySubkeys(map_kv, dispKV);
	
	//Get map array.
	new Handle:mapxArray = UMC_CreateInvalidMapArray(map_kv, umc_mapcycle, cat, true, false);
	
	if (GetConVarBool(cvar_sort))
		SortMapTrieArray(mapxArray);
	
	size = GetArraySize(mapxArray);
	for (new i = 0; i < size; i++)
	{
		mapTrie = GetArrayCell(mapxArray, i);
		GetTrieString(mapTrie, MAP_TRIE_MAP_KEY, mapBuff, sizeof(mapBuff));
		GetTrieString(mapTrie, MAP_TRIE_GROUP_KEY, groupBuff, sizeof(groupBuff));
		...
		char minPlayersOnMap[16];
		KvGetString(map_kv, "min_players", minPlayersOnMap, sizeof(minPlayersOnMap), "");
		
		char minTimeRequired[5];
		KvGetString(map_kv, "min_time", minTimeRequired, sizeof(minTimeRequired), "");
